...
render = (estado, nombre) => {
    ...
    hijo.innerHTML = `  <div class="card_actv ${estado}" onclick="eliminar();"> <h3>${nombre}</h3> <p class="">Estado:<span class="estadoSpan">${estado}</span></p></div>  `;
...
eliminar = () => {
    ...
    let check = document.getElementsByClassName('card_actv');
    for (let i = 0; i < check.length; i++) {
        check[i].addEventListener('click', function() {
            check[i].classList.add('checked');
            console.log(estadoA[i]);
            estadoA[i].estado = "checked";
            almacenadorLocalStorage();
